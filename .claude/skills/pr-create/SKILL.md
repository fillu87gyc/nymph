...
> **スクリーンショットの埋め込みは CI が自動で行う。**
> E2E ジョブが `playwright-screenshots/*.png` を GitHub Release (`screenshots` tag) にアップロードし、
> PR description の `## デモ` セクションに image タグで自動追記する。手動作業は不要。
...
- 全ジョブがグリーンになったら完了を報告する（スクリーンショットは CI が自動追記）
