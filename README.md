...
`?` でショートカット一覧が開きます（「⋯」メニューの **⌨ ショートカット一覧** からも同じものが出ます）。

| キー | 動作 |
|------|------|
| `?` | ショートカット一覧を開く / 閉じる |
| `Ctrl` / `Cmd` + `P` | Quick Open（ファイル名と本文の検索） |
| `C` | コメントパネルを開く / 閉じる |
| `T` | テーマ（ダーク / ライト）を切り替える |
| `Esc` | モーダル・メニュー・一覧を閉じる |

修飾キーなしの 1 文字（`?` / `C` / `T`）は、コメント欄や検索欄に入力しているあいだと、モーダルが開いているあいだは反応しません。日本語入力の変換中にも横取りしません。`Ctrl` / `Cmd` と一緒に押したときも反応しないので、`Ctrl+T`（新しいタブ）や `Cmd+C`（コピー）はそのまま使えます。

コメントパネルをウィジェット枠に置いている場合は常時表示になるため、`C` では開閉しません。
...
「⋯」メニューの **🖨 印刷 / PDF** で、開いている文書をそのまま印刷・PDF 保存できます（`Ctrl` / `Cmd` + `P` は Quick Open に割り当てているため、印刷はこのメニューから開きます）。
...
コメントは紙に出ません。コメント入りで配りたいときは、印刷用 CSS 込みで書き出される HTML（下の「画面からのエクスポート」または `--export`）を開いて印刷してください。

### 画面からのエクスポート

「⋯」メニューの **エクスポート** から、CLI と同じ 3 形式をブラウザに直接ダウンロードできます。ターミナルへ戻らなくても、レビューしたその画面から結果を持ち出せます。

| 項目 | 中身 | 対応する CLI |
|------|------|-------------|
| 📄 **HTML（コメント入り）** | 単体で完結する静的 HTML | `--export` |
| 📝 **Markdown（コメント入り）** | 本文の隣に指摘が引用で入った `.md` | `--annotate` |
| 📊 **CSV（コメント一覧）** | 1 コメント 1 行の表 | `nymph export` |

- ファイル名は `<元の名前>-review.<拡張子>`（`report.md` → `report-review.md`）です。元ファイルと同じ拡張子になる Markdown でも取り違えません
- 生成物は**サーバー側のディスクに残しません**。書き出し先を決めるのはブラウザ（保存ダイアログ / ダウンロードフォルダ）です
- **Mermaid を同梱** にチェックを入れると、HTML に描画エンジンを焼き込みます（`--export-mermaid` 相当）。既定は off です——生成物が約 3MB 大きくなるため
- CSV だけは CLI と既定が逆で、**BOM 付き**で落とします。画面からのダウンロードは行き先がほぼ表計算ソフトなので、Excel での文字化けを防ぐ側に倒しています（パイプに繋ぐ用途は CLI の `nymph export` が引き続き BOM 無しです）

中身は CLI の出力とまったく同じです（組み立てを共有しているので、経路によって解釈が変わることはありません）。それぞれの詳細は下の各節を参照してください。
