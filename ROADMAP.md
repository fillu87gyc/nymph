...
1. **ウィジェット第2弾** — 枠組みができたので、新しいウィジェットを足していく（下表）
2. **ヘッドレス CLI コメント** — `nymph comment <file>:<line[-end]> "本文"` でブラウザなしにコメント追加・一覧・削除。エージェント連携（Claude Code フック）の深化
3. **その他候補（優先度低）** — frontmatter の折りたたみ表示 / GitHub Alerts 対応 / レビューのコピー形式選択（JSON に加え Markdown/Text）/ stdin パイプ（`cat x.md | nymph`、コンテンツハッシュ重複排除）/ スクロール位置復元・見出しディープリンク

### ウィジェット候補（第2弾以降）

第1弾で移設した4枚（タブ / エクスプローラー / アウトライン / コメント）に続けて置けるもの。
`WIDGET_IDS` に id を足し、`WIDGET_META` にラベルと `grows` を書き、`App.tsx` の
`renderWidget` に分岐を1つ足せば枠に載る。

| ウィジェット | 内容 | 流用できる既存資産 | コスト |
|---|---|---|---|
| **ミニマップ** | 文書全体の縮小表示＋ビューポート枠＋クリックジャンプ。コメント位置・diff 位置をマーカーで重ねられるのが nymph 的な旨味 | `parseBlocks`（ブロック種別と行範囲）。ピクセル忠実な縮小より棒グラフ状の疑似ミニマップが安く速い | 大 |
| **検索結果** | Quick Open の一時表示を常設パネル化（VSCode の Search view 相当） | `GET /search`・`useSearch` | 中 |
| **最近 / ブックマーク** | ツールバーのメニューにしかない履歴・ブックマークを常設リスト化 | `useRecent` / `useBookmarks` | 小 |
| **差分サマリ** | チェックポイントからの ±行数・変更ブロック一覧＋ジャンプ | `useDiff`・`diffRows` | 中 |
| **図（Mermaid）一覧** | 文書内の図のサムネイル一覧＋ジャンプ。AI 生成 Mermaid のレビューという主用途と相性が良い | `ContentArea` の mermaid 描画 | 中 |
| **用語集（Dict）** | `.nymph/dict.json` の用語一覧・検索・本文内出現箇所へジャンプ（今はホバーツールチップだけで一覧する手段がない） | `useDict`・`termHighlight` | 中 |
| **文書統計** | 文字数・見出し数・コードブロック数・推定読了時間 | `extractToc`・`parseBlocks` | 小 |
| **タスク一覧** | `- [ ]` チェックボックスを集約してジャンプ | `parseBlocks` | 小 |
| **リンク / 画像一覧** | 相対リンクの生死チェック含む | — （リンク切れ検出はサーバー側 API が要る） | 中 |
| **frontmatter / メタ情報** | YAML frontmatter を整形表示（上の「frontmatter の折りたたみ表示」と統合するか要整理） | `yaml` | 小 |

**枠組み側の未着手**
- 枠の幅をドラッグで変える（今は左240px / 右220px 固定）。`ContentResizer`（本文左右端の幅ドラッグ）と操作が並ぶので責務分担の整理が要る
- 下スロット（今回は「左右のみ」で確定。コメントの下ドックは既定位置として残している）
- 配置画面のプレビューは枠の中身を名前のチップで並べるだけ。ウィジェットが増えたら、実物の縮小プレビューや「今は非表示」（例: ルート未指定のエクスプローラー）の注記があると親切
