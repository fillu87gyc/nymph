...
- Markdown ホットリロード（SSE）
- Mermaid ダイアグラムレンダリング + draw.io エクスポート
- ブロック単位インラインコメント（行番号紐づけ・JSON 保存）
- テキスト選択コメント（選択範囲を context として記録）
- コメント編集 / スクロール同期
- 複数ファイルウォッチ（glob・ディレクトリ、タブ切替 UI）
- チェックポイント + ± diff ハイライト
- ドラッグ & ドロップ（ファイルをブラウザにドロップして監視切替）
- ダーク / ライトテーマ切替
- レビューのクリップボードコピー
- Claude Code hook 連携（Edit ツール使用時にコメント行番号を自動リマップ）
...
| # | 機能 | 優先度 |
|---|------|--------|
| 1 | **npm 公開** — `bun publish` で `bunx nymph` が使えるようにする | 高 |
| 2 | **コードシンタックスハイライト** — fenced code block を言語別着色 | 中 |
| 3 | **キーボードショートカット** — `?` でショートカット一覧表示、`C` でコメントパネル開閉、`T` でテーマ切替 | 中 |
| 4 | **差分ビュー** — `nymph a.md b.md --diff` で 2 ファイルの追加 / 削除行をハイライト | 中 |
| 5 | **コメント検索 / フィルタ** — コメントパネル内で文字列検索 | 低 |
| 6 | **設定ファイル対応** — `.nymph.toml` でポート・テーマ・ポーリング間隔をプロジェクト単位で指定 | 低 |
