...
import { renderMarkdown, restoreIndicators, applyDiffHighlight } from '../lib/markdown.ts';
...
interface ContentAreaProps {
  ...
  onAddComment: (ls: number, le: number, displayCtx: string, blockType: string, context: any, selectionOffset: number | null) => void;
  onOpenDrawio: (code: string) => void;
  contentRef: React.RefObject<HTMLDivElement | null>;
}

export function ContentArea({
  source, comments, diffMode, diffData,
  onAddComment, onOpenDrawio, contentRef,
}: ContentAreaProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      await renderMarkdown(container, welcome, source, onAddComment, onOpenDrawio);
    ...
    <div id="content" ref={contentRef as React.RefObject<HTMLDivElement>}>
      <div id="welcome" ref={welcomeRef}>
        <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
          ...
          <path d="M12 13h16M12 19h16M12 25h10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
