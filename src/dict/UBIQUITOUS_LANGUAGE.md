...
> このドキュメントは `src/dict` モジュールで使われる概念・用語を一覧化したものです。
...
辞書に登録される概念の名前。括弧表記（後述）を除いたプレーンテキスト。
...
エントリの出所を表す識別子。`nymph.yml` の `sources[].name` が入る。
...
モジュールの動作を制御する YAML ファイル。デフォルトはプロジェクトルートの `.nymph/config.yml`。
...
`dict.json` の書き込み先。省略時は `.nymph/dict.json`。
...
ノードが表す Markdown 要素の種類。対応する型と意味の対応：
...
### 疑似クラス `:contains()` *pseudo-class*

`text` フィールドに指定文字列を**部分一致**で含むノードのみを通すフィルタ。シングルクォート・ダブルクォートどちらも使用可。

```
h2:contains('ユビキタス言語')   → テキストに "ユビキタス言語" を含む h2
h2:contains("ユビキタス言語")   → 同上（ダブルクォート形式）
```

### コンビネータ *combinator*

2 つのセレクタの**位置関係**を定義する記号。
...
### `select(nodes, selector)` 関数

木全体に対してセレクタを適用し、マッチしたノードの配列を返す。

### `selectRelative(termNode, relSelector, rootNodes)` 関数

`term` アンカーを基準に相対セレクタを評価し、定義ノードの配列を返す。`rootNodes` は `~` / `+` がルートレベルで機能するために必要。
...
### Markdown アダプタ `markdown`

Markdown テキストを木に変換し、セレクタで用語・定義を抽出するアダプタ。

- `buildTree` → `select` → `selectRelative` の順で処理する
- エイリアスは括弧表記から自動抽出される
...
> **注意:** JSON アダプタの `rules` はフィールド名であり、セレクタ構文は使用しない。
...
`dict.json` の `updatedAt` と TTL を比較した状態。`age >= ttlMs` であれば **stale**（再ビルドが必要）、それ以前であれば **fresh**（再利用可能）。

### `isStale(dictFile, config)` 関数

`updatedAt` が空・不正な日付・TTL を超過した場合に `true` を返す。

### `parseTtl(ttl)` 関数

`"24h"` `"30m"` 形式の文字列をミリ秒に変換する。`0` または不正な形式はデフォルト（86400000ms = 24h）にフォールバックする。

### デバッグ成果物 *debug artifacts*

`--debug` フラグ使用時に `.nymph/debug/` 以下に出力される中間ファイル群。

| ファイル | 内容 |
|---|---|
| `.nymph/debug/tree/<name>.json` | Markdown から構築したノード木（parent 参照なし） |
...
デバッグ時に `.nymph/raw/<name>.txt` へ書き出すフェッチ後の生テキスト。

---

## 7. ビルドパイプライン

### `buildDict(options)` 関数
...
| フィールド | 意味 | デフォルト |
|---|---|---|
| `configPath` | nymph.yml のパス | （必須） |
| `outPath` | dict.json の出力先 | `config.dict.out` か `.nymph/dict.json` |
| `debug` | デバッグ成果物を出力するか | `false` |
| `debugDir` | デバッグ出力先ディレクトリ | `.nymph/debug` |
...
### spawn（シェルなし実行）*shell-free spawn*
...
DictEntry[]  ──writeDictFile──▶  dict.json (DictFile)
                                       ↑
