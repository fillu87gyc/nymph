...
test.describe('コメントパネルのリサイズ', () => {
  ...
  test('パネル高さが localStorage に保存される', async ({ page }) => {
    ...
    await page.waitForFunction(
      () => localStorage.getItem('thyrs-panel-height') !== null,
      { timeout: 2000 },
    );
    const saved = await page.evaluate(() =>
      localStorage.getItem('thyrs-panel-height'),
    );
  ...
  test('保存したパネル高さがリロード後に復元される', async ({ page }) => {
    ...
    await page.evaluate(() =>
      localStorage.setItem('thyrs-panel-height', '380'),
    );
    await page.reload();
    await expect(
      page.locator('#content [data-testid="md-block"]').first(),
...
test.describe('テーマ切替', () => {
  ...
  test('テーマが localStorage に保存される', async ({ page }) => {
    await page.locator('#btn-theme').click();
    const saved = await page.evaluate(() =>
      localStorage.getItem('thyrs-theme'),
    );
    ...
    const orphanedComment = [
      {
        ...
        context: '【THYRS_TEST_ORPHAN_DOES_NOT_EXIST_XYZ_99999】',
        selection_offset: 0,
        text: '孤立コメント',
      },
    ];
