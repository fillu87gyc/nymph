...
const THYRS_ROOT = process.cwd();

/**
 * テスト専用の XDG_DATA_HOME。
 * 本物の ~/.config/thyrs を汚染しないようワーカーごとに分離する。
 */
const TEST_XDG = join(tmpdir(), `thyrs-cli-e2e-${process.pid}`);
...
test.beforeAll(() => {
  ...
  try {
    for (const cfgPath of [
      'tests/fixtures/dict/thyrs.yml',
      'tests/fixtures/dict/thyrs-aliases.yml',
      'tests/fixtures/dict/thyrs-json.yml',
    ]) {
...
test.describe('thyrs dict build CLI', () => {
  ...
  test('fixture thyrs.yml → dict.json が生成される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/thyrs.yml --out ${outPath}`,
      { cwd: THYRS_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
  ...
  test('--debug フラグで中間ファイルが出力される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/thyrs.yml --out ${debugOut} --debug --debug-dir ${debugDir}`,
      { cwd: THYRS_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
...
test.describe('thyrs dict build CLI — markdown aliases', () => {
  ...
  test('thyrs-aliases.yml → dict.json が生成される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/thyrs-aliases.yml --out ${outPath}`,
      { cwd: THYRS_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
    expect(existsSync(outPath)).toBe(true);
    const dict = JSON.parse(readFileSync(outPath, 'utf-8'));
...
test.describe('thyrs dict build CLI — json adapter', () => {
  ...
  test('thyrs-json.yml → dict.json が生成される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/thyrs-json.yml --out ${outPath}`,
      { cwd: THYRS_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
...
test.describe('thyrs dict allow / build — 承認フロー', () => {
  const outPath = '/tmp/test-dict-consent.json';
  const cfgPath = 'tests/fixtures/dict/thyrs.yml';

  // このブロックは独立した一時 XDG を使う
  const CONSENT_XDG = join(tmpdir(), `thyrs-consent-flow-${process.pid}`);
  ...
  test('未承認の場合 thyrs dict build は exit code 1 で失敗する', () => {
    ...
    expect(result.stderr).toContain('thyrs dict allow');
  });

  test('thyrs dict allow に y を渡すと承認される', () => {
    const result = spawnSync(
      'bun',
  ...
  test('承認後は thyrs dict build が成功する', () => {
    const result = spawnSync(
      'bun',
      [
        'run',
  ...
  test('thyrs dict allow に N を渡すとキャンセルされビルドもできない', () => {
    const REJECT_XDG = join(tmpdir(), `thyrs-reject-${process.pid}`);
