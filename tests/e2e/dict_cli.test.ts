...
const NAIAD_ROOT = process.cwd();

/**
 * テスト専用の XDG_DATA_HOME。
 * 本物の ~/.config/naiad を汚染しないようワーカーごとに分離する。
 */
const TEST_XDG = join(tmpdir(), `naiad-cli-e2e-${process.pid}`);
...
test.beforeAll(() => {
  ...
  try {
    for (const cfgPath of [
      'tests/fixtures/dict/naiad.yml',
      'tests/fixtures/dict/naiad-aliases.yml',
      'tests/fixtures/dict/naiad-json.yml',
    ]) {
...
test.describe('naiad dict build CLI', () => {
  ...
  test('fixture naiad.yml → dict.json が生成される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/naiad.yml --out ${outPath}`,
      { cwd: NAIAD_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
  ...
  test('--debug フラグで中間ファイルが出力される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/naiad.yml --out ${debugOut} --debug --debug-dir ${debugDir}`,
      { cwd: NAIAD_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
...
test.describe('naiad dict build CLI — markdown aliases', () => {
  ...
  test('naiad-aliases.yml → dict.json が生成される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/naiad-aliases.yml --out ${outPath}`,
      { cwd: NAIAD_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
    expect(existsSync(outPath)).toBe(true);
    const dict = JSON.parse(readFileSync(outPath, 'utf-8'));
...
test.describe('naiad dict build CLI — json adapter', () => {
  ...
  test('naiad-json.yml → dict.json が生成される', () => {
    execSync(
      `bun run src/cli.ts dict build --config tests/fixtures/dict/naiad-json.yml --out ${outPath}`,
      { cwd: NAIAD_ROOT, encoding: 'utf-8', env: cliEnv() },
    );
...
test.describe('naiad dict allow / build — 承認フロー', () => {
  const outPath = '/tmp/test-dict-consent.json';
  const cfgPath = 'tests/fixtures/dict/naiad.yml';

  // このブロックは独立した一時 XDG を使う
  const CONSENT_XDG = join(tmpdir(), `naiad-consent-flow-${process.pid}`);
  ...
  test('未承認の場合 naiad dict build は exit code 1 で失敗する', () => {
    ...
    expect(result.stderr).toContain('naiad dict allow');
  });

  test('naiad dict allow に y を渡すと承認される', () => {
    const result = spawnSync(
      'bun',
  ...
  test('承認後は naiad dict build が成功する', () => {
    const result = spawnSync(
      'bun',
      [
        'run',
  ...
  test('naiad dict allow に N を渡すとキャンセルされビルドもできない', () => {
    const REJECT_XDG = join(tmpdir(), `naiad-reject-${process.pid}`);
