...
test.describe('チェックポイント', () => {
  test('チェックポイントボタンで has-checkpoint クラスが付く', async ({ page }) => {
  ...
  test('チェックポイント設定後にトーストが表示される', async ({ page }) => {
    await page.locator('#btn-checkpoint').click();
    await expect(page.locator('#toast')).toContainText('チェックポイント', { timeout: 3000 });
  });
});

test.describe('diff 表示', () => {
  test('チェックポイント設定後にファイルを変更すると diff-changed ブロックが表示される', async ({ page }) => {
    await page.locator('#btn-checkpoint').click();
    await expect(page.locator('#btn-checkpoint')).toHaveClass(/has-checkpoint/);

    writeFileSync(
      FIXTURE,
      ORIGINAL.replace('Some content here.', 'Modified content here.\nExtra new line.'),
    );
    await expect(page.locator('#content')).toContainText('Modified content', { timeout: 5000 });

    await page.locator('#btn-diff').click();
    await expect(page.locator('#btn-diff')).toHaveClass(/active/);

    await expect(page.locator('#content .diff-changed').first()).toBeVisible({ timeout: 3000 });
  });

  test('diff ON のとき変更ブロックに diff-side-ins / diff-side-del が表示される', async ({ page }) => {
    await page.locator('#btn-checkpoint').click();
    writeFileSync(
      FIXTURE,
      ORIGINAL.replace('Some content here.', 'Replaced content.'),
    );
    await expect(page.locator('#content')).toContainText('Replaced content', { timeout: 5000 });
  ...
  test('diff ON のとき diff-side-ins に追加行テキストが含まれる', async ({ page }) => {
    await page.locator('#btn-checkpoint').click();
    writeFileSync(
      FIXTURE,
      ORIGINAL.replace('Some content here.', 'UNIQUE_INS_TEXT'),
    );
    await expect(page.locator('#content')).toContainText('UNIQUE_INS_TEXT', { timeout: 5000 });

    await page.locator('#btn-diff').click();
    await expect(page.locator('.diff-side-ins').first()).toBeVisible({ timeout: 3000 });
    await expect(page.locator('.diff-side-ins').first()).toContainText('UNIQUE_INS_TEXT');
  });
  ...
  test('diff OFF にすると diff-changed が消える', async ({ page }) => {
    ...
    await expect(page.locator('#content')).toContainText('Changed for diff off test', { timeout: 5000 });
  ...
  test('チェックポイントなしで diff ON にしても diff-changed は表示されない', async ({ page }) => {
    await page.locator('#btn-diff').click();
    await expect(page.locator('#btn-diff')).toHaveClass(/active/);
    await expect(page.locator('#content .diff-changed')).toHaveCount(0);
  });

  test('ファイルを変更していない場合は diff-changed が表示されない', async ({ page }) => {
    await page.locator('#btn-checkpoint').click();
    await page.locator('#btn-diff').click();
    await expect(page.locator('#content .diff-changed')).toHaveCount(0);
  });
