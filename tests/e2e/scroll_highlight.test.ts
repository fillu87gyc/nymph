...
async function addCommentToBlock(
  ...
  const block = page.locator('#content .md-block').nth(blockIndex);
  await block.hover();
...
test.describe('blockRefsMap 経由のスクロール', () => {
  test('コメントクリックで対応ブロックがビューポートに入る', async ({
    ...
    // コードブロック（下のほうにある）にコメントを付ける
    const codeBlock = page.locator('#content .md-block').filter({
      has: page.locator('pre'),
    });
    await codeBlock.first().hover();
    await codeBlock.first().locator('.comment-btn').click();
    await page.locator('#comment-ta').fill('code block comment');
    await page.locator('#btn-submit').click();
    await expect(page.locator('.comment-item').first()).toBeVisible({
      timeout: 3000,
    });
    ...
    // コメントクリック → コードブロックまでスクロールされる
    await page.locator('.comment-item').first().click();
    await expect(codeBlock.first()).toBeInViewport({ timeout: 3000 });
  });

  test('複数ブロックにコメントして別々にスクロールできる', async ({ page }) => {
    await addCommentToBlock(page, 0, 'first block');
    await addCommentToBlock(page, 2, 'third block');

    const items = page.locator('.comment-item');
    await expect(items).toHaveCount(2);

    // 2番目のコメントクリック → 3番目のブロックに移動
    const thirdBlock = page.locator('#content .md-block').nth(2);
    const thirdLs = await thirdBlock.getAttribute('data-ls');

    await items.nth(1).click();
    await expect(
      page.locator(`#content .md-block[data-ls="${thirdLs}"]`),
...
test.describe('ハイライト CSS クラス', () => {
  test('コメントクリックで .highlighted クラスが付く', async ({ page }) => {
    await addCommentToBlock(page, 0, 'highlight check');
  ...
  test('.highlighted は 1.4s アニメーション後に除去される', async ({
    ...
    await addCommentToBlock(page, 0, 'fade check');
  ...
  test('同時に複数ブロックが highlighted にならない', async ({ page }) => {
    await addCommentToBlock(page, 0, 'a');
    await addCommentToBlock(page, 1, 'b');
